...
1,2</sup>, 李宇轩<sup>1,2</sup>, Maryelin<sup>1,2</sup>, 谢卓然<sup>1,2</sup>
...
5. 基于指针(依托比如numpy等)，处理各个功能模块之间的数据交换，从而实现以上过程的高效耦合。另外，核心计算功能仅依赖Python标准库，从而确保模块可以在几乎任一Python版本运行。
...
1. 请只修改自己创建的文件(以避免冲突); 
2. 请尽量只上传代码，*不要上传比较大的数据* (Gitee仓库有500Mb的总量限制，且历史记录等，也会占用空间); 
