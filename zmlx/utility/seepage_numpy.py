...
from zml import Seepage, is_array, get_pointer64
...
class SeepageNumpy:
    ...
    class Cells:
        ...
        def get(self, index, buf=None):
            ...
            if np is not None:
                ...
                self.model.cells_write(pointer=get_pointer64(buf),
                                       index=index)
                return buf
        ...
        def set(self, index, buf):
            ...
            if not is_array(buf):
                ...
                assert len(buf) == self.model.cell_number
                self.model.cells_read(pointer=get_pointer64(buf, readonly=True),
                                      index=index)
    ...
    class Faces:
        ...
        def get(self, index, buf=None):
            ...
            if np is not None:
                ...
                self.model.faces_write(pointer=get_pointer64(buf),
                                       index=index)
                return buf
        ...
        def set(self, index, buf):
            ...
            if not is_array(buf):
                ...
                assert len(buf) == self.model.face_number
                self.model.faces_read(pointer=get_pointer64(buf, readonly=True),
                                      index=index)
    ...
    class Fluids:
        ...
        def set(self, index, buf):
            ...
            """
            if not is_array(buf):
                self.model.fluids_read(fluid_id=self.fluid_id, value=buf,
                                       index=index)
                return
            else:
                assert len(buf) == self.model.cell_number
