...
class RuntimeFunc:
    def __init__(self, pack_name, func_name, deprecated_name=None, deprecated_date=None):
    ...
    def get_func(self):
        if self.deprecated_name is not None:
            from zml import log
            warnings.warn(f'function "{self.deprecated_name}" will be removed after {self.deprecated_date}, '
        ...
        try:
            ...
            warnings.warn(f'meet error when import "{self.func_name}" from "{self.pack_name}". error = "{e}"')
