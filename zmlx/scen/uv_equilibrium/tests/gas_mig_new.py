...
from zmlx import *
import csv
from zmlx.scen.uv_equilibrium.uv_equilibrium_new import GasWaterUVEquilibrium
...
def balance(model: Seepage):
    ...
    for i in range(len(p)):
        data = {
            'H2O(aq)': mass['H2O'][i], 'H2O(g)': 0.0,
            'CH4(aq)': mass['CH4(aq)'][i], 'N2(aq)': mass['N2(aq)'][i], 'He(aq)': mass['He(aq)'][i],
            'CH4(g)': mass['CH4'][i], 'N2(g)': mass['N2'][i], 'He(g)': mass['He'][i],
        }
        r = eq.get_next_state(float(t[i]), float(p[i]), data)
        if r is None:
            failed.append({'time': tfc.get_time(model), 'cell': i, 'temperature': float(t[i]),
                           'pressure': float(p[i]), **{k: float(v) for k, v in data.items()}})
            continue
        t[i] = eq.last_temperature
        mass['H2O'][i] = r['H2O(aq)'] + r['H2O(g)']
        mass['CH4(aq)'][i], mass['N2(aq)'][i], mass['He(aq)'][i] = r['CH4(aq)'], r['N2(aq)'], r['He(aq)']
        mass['CH4'][i], mass['N2'][i], mass['He'][i] = r['CH4(g)'], r['N2(g)'], r['He(g)']
    if failed:
        with open('reaktoro_warning_cells.csv', 'a', newline='') as f:
            w = csv.DictWriter(f, fieldnames=failed[0].keys())
            if not csv_header_written:
                w.writeheader()
                csv_header_written = True
            w.writerows(failed)
        print(f'Reaktoro warning cells: {len(failed)}/{len(p)} = {len(failed) / len(p):.2%}')

    # step 3.
    # 将更新之后的mass重新应用到模型中
    ada.fluids('aqueous').set_attr('temperature', t)
    for name in names:
