...
裂缝处理参考 zmlx.demo.egs2：
1. 使用固定随机种子生成二维随机裂缝线段；
2. 将线段逐条加入 Dfn2；
3. 每条裂缝从起点 cell 出发，在相邻 cell 中选择最靠近裂缝线段和终点的 cell；
4. 将沿线路径上的 face 渗透率设置为 FRACTURE_PERM；
5. 不计算裂缝交点，不进行交点吸附、分段采样、网络分量或连通性分析；
6. 气体组成不归一化；不再单独显示随机裂缝网络，只保留 show5 生成的 fracture work。

裂缝仍是已有基质网格面上的等效高渗透带，
不是具有独立开度、孔隙体积和流体储量的离散裂缝单元。
"""
...
# 随机裂缝模式：
#   "fully_random"：只添加随机裂缝；
#   "random_with_backbone"：随机裂缝外，再添加三条确定性主链裂缝。
FRACTURE_MODE = "random_with_backbone"

# 固定随机种子，保证每次生成相同的裂缝几何。
RANDOM_SEED = 20260712

# 随机裂缝数量、长度范围和角度范围。
RANDOM_FRACTURE_NUMBER = 60
...
BACKBONE_FRACTURES = [
    dict(fracture_id="B1", x0=250.0, y0=450.0, x1=450.0, y1=650.0),
    dict(fracture_id="B2", x0=400.0, y0=600.0, x1=600.0, y1=400.0),
    dict(fracture_id="B3", x0=550.0, y0=350.0, x1=750.0, y1=550.0),
]

FRACTURE_PERM = 5.0e-11
Z_FRAC = 0.0
# 保留 show5 生成的 fracture work，并在场图中使用裂缝记录。
SHOW_FRACTURE_IN_FIELD_PLOTS = True
...
def create_fracture_geometry() -> List[Dict]:
    """根据 FRACTURE_MODE 返回最终裂缝线段列表。"""
    fractures = generate_random_fractures()

    if FRACTURE_MODE == "fully_random":
        pass
        pass
    elif FRACTURE_MODE == "random_with_backbone":
        for fracture in BACKBONE_FRACTURES:
            value = dict(fracture)
            value["source_type"] = "main_channel"
            fractures.append(validate_fracture(value))
    else:
        raise ValueError(
            "FRACTURE_MODE 只能为 'fully_random' 或 "
            f"'random_with_backbone'，当前为 {FRACTURE_MODE!r}。"
        )
    ...
    print("\n========== EGS 风格随机裂缝 ==========")
    print(f"裂缝模式 = {FRACTURE_MODE}")
    print(f"随机种子 = {RANDOM_SEED}")
    print(f"随机裂缝数量 = {RANDOM_FRACTURE_NUMBER}")
    print(f"最终裂缝数量 = {len(fractures)}")
...
def add_fractures_like_egs(model, source_fractures: List[Dict]) -> List[Dict]:
    ...
    # 将裂缝记录传给 show5，只保留 fracture work。
    # fracture_id 设为空字符串，避免在图中标注 R001、R002 等裂缝名称。
    display_fracture_records = [
        dict(record, fracture_id="")
        for record in fracture_records
    ]
    model.temps["fracture_records"] = (
        display_fracture_records if SHOW_FRACTURE_IN_FIELD_PLOTS else []
    )

    # 完整裂缝记录仍在模型内部保留，便于诊断和后续数据处理。
    model.temps["fracture_internal_records"] = fracture_records
    model.temps["fracture_intersections"] = []
