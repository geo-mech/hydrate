...
def create(
    ...
    if mineralization is not None:
        from zmlx.scen.mineralization_reactor import add_to_co2_model
        add_to_co2_model(model, mineralization)
    # 返回创建的模型.
    return model
...
def show(model: Seepage):
    ...
    if _has_mineralization(model):
        from zmlx.scen.mineralization_reactor import show_xz
        show_xz(
            model,
            caption='CO2矿化封存',
            fields=[
...
def solve(model, *args, extra_plot=None, slots=None, **kwargs):
    def x():
        show(model)
    ...
    if _has_mineralization(model):
        from zmlx.scen.mineralization_reactor import co2_mineralization_reaction
        slots = {
            'co2_mineralization_reaction': co2_mineralization_reaction,
            **({} if slots is None else slots),
        }
    hydrate.solve(model, *args, extra_plot=x, slots=slots, **kwargs)


def _has_mineralization(model):
    try:
        return bool(model.get_text('Mineralization'))
    except Exception:
        return False
