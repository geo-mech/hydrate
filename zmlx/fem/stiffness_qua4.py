...
def stiffness(x0, x1, x2, x3, y0, y1, y2, y3, E, nu):
    """
    ...
    ke1 = tri3(ex[0], ex[1], xm, ey[0], ey[1], ym, E, nu)
    tr1 = np.array([1, 2, 3, 4, 9, 10])
    idx = tr1-1
    K[np.ix_(idx, idx)] = K[np.ix_(idx, idx)] + ke1
    
    ke1 = tri3(ex[1], ex[2], xm, ey[1], ey[2], ym, E, nu)
    tr2 = np.array([3, 4, 5, 6, 9, 10])
    idx = tr2-1
    K[np.ix_(idx, idx)] = K[np.ix_(idx, idx)] + ke1
    
    ke1 = tri3(ex[2], ex[3], xm, ey[2], ey[3], ym, E, nu)
    tr3 = np.array([5, 6, 7, 8, 9, 10])
    idx = tr3-1
    K[np.ix_(idx, idx)] = K[np.ix_(idx, idx)] + ke1
    
    ke1 = tri3(ex[3], ex[0], xm, ey[3], ey[0], ym, E, nu)
...
print( stiffness(x0=0, x1=2, x2=2, x3=0, y0=0, y1=0, y2=2, y3=2, E=200e5, nu=0.32))
