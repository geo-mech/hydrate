#  '平面应变Quad8单元应变验证：悬臂梁(L=4,H=1)，左侧固定，右上角y负方向力'

from zmlx.fem.xy import Mesh3
from zmlx.fem.dyn import create_dyn
from zmlx.fem.elements.planar_strain_quad8 import calc_stiffness
...
def create_mesh():
    ...
    # 构建角节点单元（与test_4d的Quad4四边形连通性完全相同）
    corner_elements = []
    for j in range(Ny):
        for i in range(Nx):
...
def _show(model: _Model):
    ...
    plot_on_figure(on_figure, caption='PlanarStrainQuad8')
