# ** desc = '平面应力CST单元应变验证（加密网格）：悬臂梁(L=4,H=1)，左侧固定，右上角y负方向力'
...
def _create(mesh: Mesh3):
    ...
    model.set_mesh(
        ...
        face_types=[FaceType.PlanarStressCST] * mesh.face_number,
    )
...
def _show(model: FemModel):
    ...
    plot_on_figure(on_figure, caption='PlanarStressCST (refined)')
