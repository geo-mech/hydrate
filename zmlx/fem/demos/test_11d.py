#  '平面应力Quad8单元应变验证：悬臂梁(L=4,H=1)，左侧固定，右上角y负方向力'

from zmlx.fem.xy import Mesh3
from zmlx.fem.dyn import create_dyn
from zmlx.fem.elements.planar_stress_quad8 import calc_stiffness
...
def create_mesh():
    """
    创建悬臂梁mesh (L=4, H=1, Nx=16, Ny=4)，每格一个四边形，
    然后使用enrich_with_mid_nodes转换为8节点Quad8单元
    """
    Lx, Ly = 4.0, 1.0
    Nx, Ny = 16, 4
    dx, dy = Lx / Nx, Ly / Ny

    # 创建角节点坐标（与test_5d完全相同）
    coords = []
    for j in range(Ny + 1):
        for i in range(Nx + 1):
    ...
    # 构建角节点单元（与test_5d的Quad4四边形连通性完全相同）
    corner_elements = []
    for j in range(Ny):
        for i in range(Nx):
            n0, n1 = _node(i, j), _node(i + 1, j)
            n2, n3 = _node(i, j + 1), _node(i + 1, j + 1)
            corner_elements.append([n0, n1, n3, n2])

    # 补全边中点，得到8节点Quad8单元
    enriched, coords, _ = enrich_with_mid_nodes(corner_elements, coords, 'quad8')
...
class _Model:
    """
    有限元模型（Mesh3不支持8节点面，直接使用DynSys组装）
    """
    ...
    def __init__(self, coords, enriched, E=1.0, mu=0.2, thickness=1.0, density=1.0):
        ...
        for elem in enriched:
            ...
            c3 = np.array(coords[elem[3]], dtype=float)
            v1, v2 = c1 - c0, c3 - c0
            area1 = 0.5 * abs(v1[0] * v2[1] - v1[1] * v2[0])
            v1, v2 = c1 - c2, c3 - c2
            area2 = 0.5 * abs(v1[0] * v2[1] - v1[1] * v2[0])
            area = area1 + area2
...
def _draw_mesh(ax, enriched, vx, vy, n_corner=4):
    """
...
def _show(model: _Model):
    ...
    def on_figure(figure):
        ...
        _draw_mesh(ax, enriched, vx, vy, n_corner=4)
    ...
    plot_on_figure(on_figure, caption='PlanarStressQuad8')
