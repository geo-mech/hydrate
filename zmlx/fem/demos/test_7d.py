# ** desc = 'C3D8六面体单元应变验证：悬臂梁(L=4,H=1,W=0.5)，左侧固定，右上角y负方向力'

from zmlx.exts import Mesh3
from zmlx.fem.xyz import FemModel
from zmlx.fem.elements.c3d8 import calc_strain
import numpy as np


def create_mesh():
    """
    创建悬臂梁mesh (L=4, H=1, W=0.5, Nx=20, Ny=4, Nz=2)，每格一个六面体
    """
    ...
    for k in range(Nz):
        for j in range(Ny):
            for i in range(Nx):
                ...
                f_bottom = mesh.add_face(
                    [_link(n0, n1), _link(n1, n2), _link(n2, n3), _link(n3, n0)])
                f_top = mesh.add_face(
                    [_link(n4, n5), _link(n5, n6), _link(n6, n7), _link(n7, n4)])
                f_front = mesh.add_face(
                    [_link(n0, n1), _link(n1, n5), _link(n5, n4), _link(n4, n0)])
                f_back = mesh.add_face(
                    [_link(n3, n2), _link(n2, n6), _link(n6, n7), _link(n7, n3)])
                f_left = mesh.add_face(
                    [_link(n0, n3), _link(n3, n7), _link(n7, n4), _link(n4, n0)])
                f_right = mesh.add_face(
                    [_link(n1, n2), _link(n2, n6), _link(n6, n5), _link(n5, n1)])

                mesh.add_body([f_bottom, f_top, f_front, f_back, f_left, f_right])
...
def _is_face_on_surface(face, axis, target, eps):
    for node in face.nodes:
        if abs(node.pos[axis] - target) > eps:
            return False
    return True
...
def _surface_faces_3d(mesh, model, eps):
    ...
    for axis, target in surfaces_def:
        for face in mesh.faces:
            if _is_face_on_surface(face, axis, target, eps):
                verts = []
                vals = []
                for node in face.nodes:
                    i = node.index
                    verts.append([node.pos[0] + dx[i],
                                  node.pos[1] + dy[i],
                                  node.pos[2] + dz[i]])
                    vals.append(disp[i])
                # 四边形拆为两个三角形
                if len(verts) == 4:
                    patches.append([verts[0], verts[1], verts[2]])
                    patches.append([verts[0], verts[2], verts[3]])
                    c = cmap(norm(np.mean(vals)))
                    colors.append(c)
                    colors.append(c)
                else:
                    patches.append(verts)
                    colors.append(cmap(norm(np.mean(vals))))
...
def _show(model: FemModel):
    ...
    plot_on_figure(on_figure1, caption='C3D8 mid-plane')
    ...
    if patches:
        ...
        def on_figure3(figure):
            ...
            ax.set_title('C3D8: 3D surface total displacement')
        ...
        plot_on_figure(on_figure3, caption='C3D8 3D surface')
