...
def calc_best_layout(
    ...
    """
    计算最优的子图布局（行数n_rows和列数n_cols）. 此函数调用calculate_subplot_layout函数.
    ...
    Returns:
        tuple : (n_rows, n_cols)
            最优的行数和列数
    """
    return calculate_subplot_layout(num_plots, subplot_aspect_ratio=subplot_aspect_ratio, fig=fig)
