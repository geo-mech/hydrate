# -*- coding: utf-8 -*-


from zmlx import *
from zmlx.plt.show_fn2 import show_fn2


def main():
    sol2 = DDMSolution2()
    manager = InfManager2()
    network = FractureNetwork2()

    # 多层的渗流模型
    layer_n = 10
    h_sum = 50
    layer_h = h_sum / layer_n
    layers = []
    while len(layers) < layer_n:
        layers.append(Seepage())
    layer_c = layers[int(layer_n / 2)]

    # 用于多层之间的连接
    pip = Seepage()
    for i in range(layer_n):
        pip.add_cell()
        if i > 0:
            pip.add_face(pip.get_cell(i - 1), pip.get_cell(i))

    fw_max = 0.0005

    lave = 1
    ca_p = 0
    ca_ny = 1
    ca_l = 2
    ca_h = 3
    ca_g = 4
    ca_mu = 5
    stress = Tensor2(xx=-1e6, yy=-1.5e6)

    kic = Tensor2(xx=1e6, yy=1e6)
    injs = [(-4, 0), (4, 0)]
    fa_id = 0
    fa_tmp = 1

    for x, y in injs:
        network.add_fracture(pos=(x, y - 1, x, y + 1), lave=lave)
        layer_c.add_injector(pos=(x, y, 0), radi=lave * 3,
                             fluid_id=0,
                             opers=[(0, layer_h * 0.0001)])

    def save_txt(path):
        with open(make_parent(path), 'w') as file:
            for f in network.get_fractures():
                cell_id = int(f.get_attr(fa_id))
                if 0 <= cell_id < 10000000:
                    x0, y0, x1, y1 = f.pos
                    area = get_distance((x0, y0), (x1, y1)) * layer_h
                    for i in range(layer_n):
                        z0 = (i - layer_n / 2) * layer_h
                        z1 = z0 + layer_h
                        cell = layers[i].get_cell(cell_id)
                        fp = cell.get_attr(ca_p)
                        fv = cell.fluid_vol
                        thick = fv / area
                        file.write(f'{x0}  {x0}  {x1}  {x1}  {y0}  {y0}  {y1}  {y1}   {z0}  {z1}  {z0}  {z1}  {fp}  {thick}\n')

    solver = ConjugateGradientSolver(tolerance=1.0e-12)

    dt = 30.0 / layer_n
    for step in range(2000 * layer_n):
        gui.break_point()
        print(f'step = {step}. frac_n = {network.frac_n}')

        for f in network.get_fractures():
            f.h = h_sum

        network.copy_attr(fa_tmp, fa_id)
        for layer_id in range(layer_n):
            lay = layers[layer_id]
            network.copy_attr(fa_id, fa_tmp)
            Hf2Alg.update_seepage_topology(seepage=lay, network=network, fa_id=fa_id)
            Hf2Alg.update_seepage_cell_pos(seepage=lay, network=network, fa_id=fa_id,
                                           coord=Coord3(origin=(0, 0, (layer_id - layer_n / 2) * layer_h)))
            Hf2Alg.update_cond(seepage=lay, network=network, fa_id=fa_id, fw_max=fw_max, layer_dh=layer_h)
            # print(f'g1 = {np.mean(lay.numpy.faces.cond)}')
            Hf2Alg.update_length(seepage=lay, ca_l=ca_l, network=network, fa_id=fa_id)
            ca = lay.numpy.cells
            ca.v0 = 0
            ca.k = ca.get(ca_l) * layer_h * 1.0e-7
            lay.iterate(dt=dt, ca_p=ca_p, solver=solver)

        for lay in layers:
            ca = lay.numpy.cells
            ca.set(ca_h, layer_h)
            Hf2Alg.update_cond(seepage=lay, ca_g=ca_g, ca_l=ca_h, ca_h=ca_l, fw_max=fw_max)
            # print(f'g2 = {np.mean(lay.numpy.cells.get(ca_g))}')
        Hf2Alg.exchange_fluids(layers=layers, pipe=pip, dt=dt, ca_g=ca_g, ca_fp=ca_p)

        manager.update_matrix(network, sol2, stress, 5)
        manager.update_boundary(layer_c, fa_id=fa_id, fh=layer_h)
        manager.update_disp()
        network.extend(kic=kic, sol2=sol2, has_branch=False, lave=lave)

        if step % (20 * layer_n) == 0:
            show_fn2(network=network, seepage=layer_c, ca_c=ca_p, w_max=6, caption='Fracture', fa_id=fa_id)
            save_txt(make_fpath(r'C:\Users\zhaob\Desktop\fracture-3d\data', step))


if __name__ == '__main__':
    gui_exec(main, close_after_done=False)
