"""含溶解 H₂ 的水溶液密度（基于 Reaktoro + PHREEQC 数据库）."""
...
def h2o_h2_density(w, P=10.0e6, T=300.0):
    """获取含溶解 H₂ 的水溶液密度.

    H₂ 分子极小，溶解度较低。

    Args:
        w: H₂ 的质量分数，m(H₂) / (m(H₂) + m(H₂O))，范围 [0, 1)
        P: 压力 (Pa)，默认 10 MPa
        T: 温度 (K)，默认 300 K
...
def h2o_h2_solubility(P=10.0e6, T=300.0):
    """获取 H₂ 在水中的饱和溶解度（质量分数）.
...
if __name__ == "__main__":
    ...
    gui.execute(lambda: plot_density_ratio("H2", h2o_h2_density, w_max=0.0002, color="deepskyblue",
                                            fn_solubility=h2o_h2_solubility),
