"""CoolProp 流体物性的 T-P 云图绘制.

提供 plot_contours 函数，生成多行等值线图（左列填充、右列等值线）。
...
def plot_contours(label, fn_density, fn_viscosity=None, fn_specific_heat=None,
                  t_range=(280, 500), p_range=(1e5, 30e6),
                  n_t=23, n_p=21):
    """绘制流体密度、粘度、比热的 T-P 云图.

    布局：第一行为密度，后续行按需追加粘度、比热。
    每行左侧为填充图 (contourf)，右侧为等值线 (contour)。

    Args:
        label: 流体名称（如 'CH4'）
        fn_density: 密度函数 fn(P, T) → kg/m³
        fn_viscosity: 粘度函数 fn(P, T) → Pa·s（可选）
    ...
    ZZ_v = _grid(lambda P, T: fn_viscosity(P, T) * 1e6) if fn_viscosity is not None else None
    ZZ_c = _grid(fn_specific_heat) if fn_specific_heat is not None else None

    rows = 1 + (1 if ZZ_v is not None else 0) + (1 if ZZ_c is not None else 0)
    ...
    def on_figure(fig):
        ...
        ax.set_title(f"{label} Density (CoolProp)")
        ...
        if ZZ_v is not None:
            r += 1
            ax = axes[r, 0]
            cf = ax.contourf(TT, PP / 1e6, ZZ_v, levels=20, cmap="inferno")
            fig.colorbar(cf, ax=ax, label="Viscosity (uPa.s)")
            ax.set_xlabel("Temperature (K)")
            ax.set_ylabel("Pressure (MPa)")
            ax.set_title(f"{label} Viscosity (CoolProp)")

            ax = axes[r, 1]
            cs = ax.contour(TT, PP / 1e6, ZZ_v, levels=15, colors=lc, linewidths=0.6)
            ax.clabel(cs, inline=True, fontsize=7)
            ax.set_xlabel("Temperature (K)")
            ax.set_ylabel("Pressure (MPa)")
            ax.set_title(f"{label} Viscosity contours (uPa.s)")
        ...
        if ZZ_c is not None:
            ...
            ax.set_xlabel("Temperature (K)")
            ax.set_ylabel("Pressure (MPa)")
            ax.set_title(f"{label} Specific heat (CoolProp)")
    ...
    plot(on_figure, caption=f"{label} (CoolProp)")
