...
from zmlx.exts import Seepage, np, const_f64_ptr, f64_ptr
from zmlx.tfc._base import as_numpy
from zmlx.tfc._base import get_face_sum, get_face_diff, get_face_left, \
    get_face_right
...
def get_face_density_diff(model: Seepage, fid0, fid1):
    ...
    get_face_sum(
        model, ca=const_f64_ptr(ca), fa=f64_ptr(fa))
    return fa / 2
...
def get_face_fv(model: Seepage, fid0, gra):
    ...
    get_face_left(
        model, ca=const_f64_ptr(ca0), fa=f64_ptr(fa0))

    # 右侧的流体0
    fa1 = np.zeros(shape=model.face_number, dtype=float)
    get_face_right(
        model, ca=const_f64_ptr(ca0), fa=f64_ptr(fa1))
...
def iterate(model: Seepage, dt: float, fid0=None, fid1=None, rate=1.0):
    ...
    if fid0 is not None and fid1 is not None:
        model.diffusion(
            ...
            pg=const_f64_ptr(cond), lg=len(cond),
            ppg=const_f64_ptr(gra), lpg=len(gra),
            ds_max=0.2)
