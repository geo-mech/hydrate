from zmlx.alg import join_paths, make_fname
from zmlx.exts import Seepage
from zmlx.plt import tricontourf
from zmlx.tfc import _base as seepage
...
def show_cells(
    ...
    x = seepage.get_cell_pos(model=model, dim=dim0, mask=mask)
    y = seepage.get_cell_pos(model=model, dim=dim1, mask=mask)

    kw = dict(title=f'time = {seepage.get_time(model, as_str=True)}')
    kw.update(opts)

    year = seepage.get_time(model) / (365 * 24 * 3600)

    if show_p:  # 显示压力
        v = seepage.get_cell_pre(model, mask=mask) / 1e6
        tricontourf(
    ...
    if show_t:  # 显示温度
        v = seepage.get_cell_temp(model, mask=mask)
        tricontourf(
    ...
    if not isinstance(show_s, list):
        if show_s:  # 此时，显示所有组分的饱和度
            show_s = seepage.list_comp(model, keep_structure=False)  # 所有的组分名称

    if isinstance(show_s, list):
        if use_mass:  # 此时，显示质量饱和度
            get = seepage.get_cell_fm
        else:
            get = seepage.get_cell_fv
