...
def iterate(*local_opts: Union[Seepage, dict], pool: Optional[ThreadPool] = None, **global_opts) -> int:
    ...
    _step.iterate(*models)

    # 迭代流体的性质
    _fluid.iterate(*models, pool=pool)

    # 流体注入
    _inj.iterate(*models, pool=pool)

    # 尝试修改边界的压力，从而使得流体生产 (使用模型内部定义的time)
    #   since 2024-6-12
    _prod.iterate(*models)

    # 对流体进行加热
    _heating.iterate(*models)

    # 备份固体
    #  注意，在solid_buffer.backup和solid_buffer.restore之间，最后一种物质不存在
    _solid.backup(*models, pool=pool)
    ...
    # 执行新版本的扩散操作
    _diff.iterate(*models, pool=pool, recommend_dt=True)

    # 恢复备份的固体
    _solid.restore(*models, pool=pool)
...
def print_cells(path: Optional[str], model: Optional[Seepage], ca_keys=None, fa_keys=None,
    ...
    注意：
        ...
        后面的饱和度为质量的比例 （否则为体积）.
    """
    if not isinstance(model, Seepage) or path is None:
        return
...
def _prepare_model(
        model: Optional[Seepage] = None,
        folder: Optional[str] = None,
        fname: Optional[str] = None
) -> Tuple[Optional[Seepage], Optional[str]]:
    if model is None:
...
def solve(
    ...
    time0: float = get_time(model)
    step0: int = get_step(model)

    def get_ratio():
        """
        估计计算完成的比例
        """
        r1 = (get_time(model) - time0) / max(1.0e-10, time_max - time0)
        r2 = (get_step(model) - step0) / max(1, step_max - step0)
        return min(1.0, max(r1, r2, 0.0))
    ...
    def gui_show():
        ...
        if gui.exists():
            show_attrs(
                step=dict(name='步数', value=get_step(model)),
                dt=dict(name='步长', value=get_dt(model, as_str=True)),
                time=dict(name='时间', value=get_time(model, as_str=True)),
            )
            progress(label='计算进度', val_range=[0, 1000], value=round(get_ratio() * 1000))
