...
def create_model(
    ...
    dfn.add_frac(
        ...
        lengths=np.linspace(fl_min, fl_max, 100),
        p21=p21,
    )
    ...
    # 导出裂缝坐标数据
    np.savetxt(os.getcwd() + "/fracture.txt", fractures)
    ...
    for x0, y0, x1, y1 in dfn.get_fractures():
        ...
        def get_dist(cell_pos):
            return seg_point_distance(
                [[x0, y0], [x1, y1]], cell_pos[0:2]
            ) + get_distance(cell_pos, cell_end.pos)
    ...
    model.add_injector(
        cell=cell, fluid_id=0, flu=flu, pos=pos, radi=2, value=vol_day / (3600 * 24)
    )

    # 添加虚拟Cell用于产出
    virtual_cell = seepage.add_cell(
        model,
        pos=[x_max, y_max, 1000.0],
        porosity=1.0,
        pore_modulus=100e6,
        vol=1.0e6,
        temperature=temp,
...
def data_save(model, folder, step):
    assert isinstance(model, Seepage)
    x = model.numpy.cells.x
    y = model.numpy.cells.y
    # 压力
    ...
    # 岩石温度
    r_t = model.numpy.cells.get(model.get_cell_key("temperature"))
    # 流体温度
    t = model.numpy.fluids(0).get(index=model.reg_flu_key("temperature"))
    combined_array = np.column_stack((x, y, p, r_t, t))
    path = make_fpath(folder + "data", step=step)
    np.savetxt(path, combined_array)
...
def solve(model: Seepage, time_max=3600 * 24 * 365 * 30, folder=None, day_save=30.0):
    ...
    iterate = GuiIterator(
        seepage.iterate, lambda: plot_cells(model, folder=path.join(folder, "figures"))
    )
    ...
    # 存储节点时间初始化
    t_old = 0
    data_save(model, folder, seepage.get_step(model))
    # 迭代到给定的时间
    ...
    while seepage.get_time(model) < time_max:
        ...
        time_now = seepage.get_time(model)

        # 间隔存储数据
        if time_now - t_old > day_save * 3600 * 24:
            print(time_now,t_old)
            data_save(model, folder, step)
            t_old = time_now
...
def _test3():
    ...
    执行一个默认的求解过程，并且保存数据.
    """
    execute(folder="../Inj_T/400/", day_save=365, t_inj=400)
