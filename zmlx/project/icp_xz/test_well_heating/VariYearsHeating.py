...
def run_case(years_heating):
    folder = os.path.join(os.getcwd(), 'VariYearsHeating', f'{years_heating}')
    well_heating(folder=folder, years_heating=years_heating)


def main(argv):
    if len(argv) == 1:
        for years_heating in np.linspace(0.0, 15.0, 16):
