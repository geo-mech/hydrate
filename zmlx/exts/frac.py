...
class Fracture2:
    ...
    core.use(c_double, 'frac2_get_pos',
             c_void_p, c_size_t)
    ...
    core.use(c_size_t, 'frac2_get_uid',
             c_void_p)

    @property
    def uid(self):
        """
        裂缝的全局ID。在程序启动之后，裂缝的ID将会是唯一的，并且是递增的。
        因此uid的数值越大，则表明裂缝创建得越晚。 另外，在裂缝扩展
        的过程种，也可以用这个uid来判断，哪些裂缝是新的。
        """
        return core.frac2_get_uid(self.handle)

    core.use(None, 'frac2_set_ds',
             c_void_p, c_double)
    core.use(c_double, 'frac2_get_ds',
             c_void_p)
    ...
    core.use(None, 'frac2_set_dn',
             c_void_p, c_double)
    core.use(c_double, 'frac2_get_dn',
             c_void_p)
    ...
    core.use(None, 'frac2_set_h',
             c_void_p, c_double)
    core.use(c_double, 'frac2_get_h',
             c_void_p)
    ...
    core.use(None, 'frac2_set_fric',
             c_void_p, c_double)
    core.use(c_double, 'frac2_get_fric',
             c_void_p)
    ...
    core.use(None, 'frac2_set_p0',
             c_void_p, c_double)
    core.use(c_double, 'frac2_get_p0',
             c_void_p)
    ...
    core.use(None, 'frac2_set_k',
             c_void_p, c_double)
    core.use(c_double, 'frac2_get_k',
             c_void_p)
    ...
    core.use(c_double, 'frac2_get_attr',
             c_void_p, c_size_t)
    core.use(None, 'frac2_set_attr',
             c_void_p, c_size_t, c_double)

    def get_attr(self, index, min=-1.0e100, max=1.0e100,
                 default_val=None):
        """
        第index个自定义属性
...
class FractureNetwork2(HasHandle):
    ...
    core.use(None, 'del_fnet2',
             c_void_p)

    def __init__(self, handle=None):
        super(FractureNetwork2, self).__init__(handle, core.new_fnet2,
                                               core.del_fnet2)
    ...
    def save(self, path):
        ...
        序列化保存. 可选扩展名:
            ...
            3: .其它  二进制格式 (速度最快，体积最小，
            但Windows和Linux下生成的文件不能互相读取)
        """
        if path is not None:
    ...
    core.use(c_size_t, 'fnet2_get_frac_n',
             c_void_p)
    ...
    core.use(c_size_t, 'fnet2_get_vtx_n',
             c_void_p)
    ...
    core.use(None, 'fnet2_get_fractures',
             c_void_p, c_void_p)
    ...
    def get_fractures(self, buffer=None):
        ...
        return [Fracture2(handle=buffer[i])
                for i in range(buffer.size)]

    core.use(None, 'fnet2_adjust',
    ...
    core.use(c_size_t, 'fnet2_create_branch',
             c_void_p, c_void_p, c_void_p,
             c_double)
    core.use(c_size_t, 'fnet2_extend_tip',
             c_void_p, c_void_p, c_void_p,
             c_double, c_double, c_double, c_double)
    core.use(c_size_t, 'fnet2_extend_tip3',
             c_void_p,
             c_void_p, c_double, c_double,
    ...
    def extend(self, kic=None, sol2=None, lave=None,
               dn_min=1.0e-7, lex=0.3,
               dangle_max=10.0,
        ...
        尝试扩展裂缝，并返回扩展的数量.
            lex:   扩展的长度与lave的比值
        注意：
            在裂缝扩展的过程中，裂缝单元的数量可能并不会发生改变.
            在扩展的时候，会首先将尖端的单元拉长。只有
            当这个尖端的裂缝的长度被拉长到不得不分割的时候，
            才会被分成两个裂缝单元。在这个尖端被分割的时候，
            裂缝的数据(包括所有的属性)，将会被分割之后的两个
            单元所共同继承. 因此，当裂缝单元通过fa_id属性
            来定义裂缝单元对应的Cell的id的时候，在裂缝扩展
            的时候，将可能会出现，两个单元所对应的cell为同
            一个的情况(在 update_seepage_topology 的时候，
            会考虑到这个问题，并自动对相应的Cell进行拆分).
        """
        count = 0
        if has_branch:
            ...
            count += core.fnet2_create_branch(
                self.handle, kic.handle,
                sol2.handle, lave)
        if 0.01 < lex < 0.99:
            assert isinstance(sol2, DDMSolution2)
            assert lave is not None
            if isinstance(kic, Tensor2):
                ...
                count += core.fnet2_extend_tip3(
                    self.handle, kic.handle,
                    left[0], left[1], left[2],
                    right[0], right[1], right[2],
                    z,
                    sol2.handle, dn_min, lave, lex,
                    dangle_max)
        return count

    core.use(None, 'fnet2_get_induced',
             c_void_p, c_void_p, c_void_p,
             c_double, c_double, c_double, c_double)
    ...
    def get_induced(self, pos, sol2):
        ...
        stress = Tensor2()
        if len(pos) == 2:
            core.fnet2_get_induced(
                self.handle, stress.handle, sol2.handle,
                *pos, *pos)
        else:
            assert len(pos) == 4
            core.fnet2_get_induced(
    ...
    core.use(None, 'fnet2_update_h_by_layers',
    ...
    def update_h_by_layers(self, layers, fa_id, layer_h, w_min):
        ...
        core.fnet2_update_h_by_layers(
            self.handle, layers.handle, fa_id,
            layer_h, w_min)

    core.use(None, 'fnet2_copy_attr',
    ...
    core.use(None, 'fnet2_update_dist2tip',
             c_void_p, c_size_t, c_size_t)

    def update_dist2tip(self, fa_dist, va_dist):
        """
        更新各个裂缝距离裂缝尖端的距离，并存储在属性fa_dist中。
        va_dist为各个节点距离裂缝尖端的距离，仅仅作为辅助计算的临时变量
        """
        core.fnet2_update_dist2tip(self.handle, fa_dist, va_dist)

    core.use(None, 'fnet2_update_cluster',
             c_void_p, c_size_t)
    ...
    core.use(None, 'fnet2_update_boundary',
             c_void_p, c_void_p, c_size_t,
             c_double)
    core.use(None, 'fnet2_update_boundary_by_layers',
             c_void_p, c_void_p,
             c_size_t)
    ...
    def update_boundary(self, seepage, fa_id, fh=None):
        """
        在DDM中，流体是作为固体计算的边界。
        此函数根据此刻的流体情况来更新固体计算的边界条件。
        其中:
            seepage 可以为一个或者多个Seepage类。
            当Seepage为多个时，其指针存储在PtrVector中;
            fa_id 为裂缝单元中存储的流体Cell的ID。

        注意：
            对于各个裂缝单元，将首先计算它对应的Cell(用fa_id指定)
            中的流体的体积，然后计算裂缝的长度，并使用
            给定的fh或者裂缝内存储的裂缝高度来计算裂缝的面积，
            进而计算流体的厚度。这个厚度就是对裂缝开度的一个
            非常硬的约束，也是后续计算裂缝的dn和ds的时候的边界条件.
        """
        if isinstance(seepage, Seepage):
            if fh is None:
                fh = -1  # Now, using the fracture height defined in fracture.
            core.fnet2_update_boundary(
                self.handle, seepage.handle, fa_id, fh)
        else:
            if not isinstance(seepage, PtrVector):
                seepage = PtrVector.from_objects(seepage)
            core.fnet2_update_boundary_by_layers(
                self.handle, seepage.handle, fa_id)

    core.use(None, 'fnet2_mark_tips',
             c_void_p, c_size_t)

    def mark_tips(self, fa_is_tip):
        """
        标记位于尖端的裂缝单元。对于尖端的单元，将自定义属性fa_is_tip的值设置为1，
        对于其它的单元，fa_is_tip的值设置为0.
            since 2023-9-14
        """
        core.fnet2_mark_tips(self.handle, fa_is_tip)

    core.use(None, 'fnet2_mark_length',
             c_void_p, c_size_t)
...
class InfManager2(HasHandle):
    ...
    二维裂缝的管理（建立应力影响矩阵）
    注意：
        这是一个临时变量，因此没有提供save/load函数。
        在调用update_matrix之后，矩阵会自动创建.
    """
    core.use(c_void_p, 'new_fmanager2')
    core.use(None, 'del_fmanager2', c_void_p)

    def __init__(self, handle=None):
        super(InfManager2, self).__init__(handle, core.new_fmanager2,
                                          core.del_fmanager2)

    core.use(None, 'fmanager2_update1',
             c_void_p, c_void_p, c_void_p, c_void_p,
             c_double)
    core.use(None, 'fmanager2_update2',
             c_void_p, c_void_p, c_void_p, c_void_p,
             c_double, c_double)
    core.use(None, 'fmanager2_update3',
             c_void_p, c_void_p, c_void_p, c_void_p,
             c_double, c_double, c_double,
             c_double, c_double, c_double,
             c_double, c_double)

    def update_matrix(self, network, sol, stress, dist, z=0, left=None,
                      right=None):
        """
        更新应力影响矩阵. 其中：
            dist为应力影响的范围. 当两个裂缝之间的距离大于dist的时候，
            则它们之间不会产生实时的应力影响。也就是说，
            会利用它们此刻的开度和剪切来计算应力，而这部分应力在
            后续不会随着它们开度的变化而变化。
        """
        assert isinstance(network, FractureNetwork2)
        assert isinstance(sol, DDMSolution2)
        if isinstance(stress, Tensor2):
            core.fmanager2_update1(self.handle, network.handle, sol.handle,
                                   stress.handle, dist)
        elif isinstance(stress, Tensor3Interp3):
            core.fmanager2_update2(self.handle, network.handle, sol.handle,
                                   stress.handle, z, dist)
        else:
    ...
    core.use(c_size_t, 'fmanager2_update_disp',
             c_void_p, c_double, c_double,
             c_size_t, c_double)

    def update_disp(self, gradw_max=0, err_max=0.1, iter_max=10000,
                    ratio_max=0.99):
        """
        更新裂缝单元的位移(dn和ds)
        """
        return core.fmanager2_update_disp(self.handle, gradw_max, err_max,
                                          iter_max, ratio_max)

    core.use(None, 'fmanager2_update_boundary',
             c_void_p, c_void_p, c_size_t,
             c_double)
    core.use(None, 'fmanager2_update_boundary_by_layers',
             c_void_p, c_void_p,
             c_size_t)
    ...
    def update_boundary(self, seepage, fa_id, fh=None):
        ...
        if isinstance(seepage, Seepage):
            if fh is None:
                fh = -1  # Now, using the fracture height defined in fracture.
            core.fmanager2_update_boundary(
                self.handle, seepage.handle, fa_id, fh)
        else:
            if not isinstance(seepage, PtrVector):
                seepage = PtrVector.from_objects(seepage)
            core.fmanager2_update_boundary_by_layers(
                self.handle, seepage.handle, fa_id)


class ExHistory(HasHandle):
    core.use(c_void_p, 'new_exhistory')
    core.use(None, 'del_exhistory',
             c_void_p)

    def __init__(self, path=None, handle=None):
        super(ExHistory, self).__init__(
            handle, core.new_exhistory, core.del_exhistory)
        if handle is None:
            if path is not None:
                self.load(path)

    core.use(None, 'exhistory_save',
             c_void_p, c_char_p)
    ...
    core.use(None, 'exhistory_write_fmap',
             c_void_p, c_void_p, c_char_p)
    core.use(None, 'exhistory_read_fmap',
             c_void_p, c_void_p, c_char_p)
    ...
    core.use(None, 'exhistory_record',
    ...
    core.use(c_double, 'exhistory_get_best_dt',
             c_void_p, c_double)
...
class Hf2Alg:
    ...
    @staticmethod
    def leak_off(seepage, fluid_id, dt, ca_vlk,
                 ca_vis_lk, ca_s, ca_fai, ca_k,
                 ca_pp):
        """
        计算流体的滤失. 其中fluid_id为即将滤失的流体的ID, dt为时间步长.
        ca_vlk为已经滤失的流体的体积； ca_vis_lk为已经滤失的流体的粘性；
        ca_s为滤失面积；ca_fai为滤失孔隙度；ca_k为滤失渗透率；
        ca_pp为远端的孔隙压力；
        """
        assert isinstance(seepage, Seepage)
        core.hf2_alg_leakoff(
            seepage.handle, *parse_fid3(fluid_id), dt, ca_vlk,
            ca_vis_lk, ca_s, ca_fai, ca_k, ca_pp)

    core.use(None, 'hf2_alg_update_seepage_topology',
             c_void_p, c_void_p,
             c_size_t,
             c_size_t, c_size_t, c_size_t)

    @staticmethod
    def update_seepage_topology(seepage, network, fa_id, fa_new=None,
                                cell_new=None, face_new=None):
        """
        建立和裂缝对应的流动模型.
            其中：
            fa_id为裂缝的自定义属性的ID，用以存储这个裂缝单元对应的Seepage中的Cell的ID.
            fa_new为裂缝的自定义属性的ID，当这个属性非0时，表示这是一个
            新的裂缝单元（新添加的，或者被新分隔的，均被视为新的单元）
            cell_new和face_new是seepage中cell和face的属性ID，
            用于表示这是否为新的cell或者face
            注意：
            1. 这里的seepage需要被这个裂缝系统所独占。即，这里建立的目标，
            是裂缝单元和seepage中的Cell一对一。但是，为了确保存储在
            fracture中的Cell的id不发生变化，这里不允许删除seepage中的
            Cell。当裂缝变少的时候，多余的Cell将会被标记成为孤立的Cell，
            不和其它的Cell产生流体的联通。
            2. 在创建seepage的时候，会尽量保证Cell中的流体和pore不发生变化。
            如果有Cell被两个裂缝所共有，那么，将会把Cell中的数据按照
            两条裂缝的长度的比例分配给两个fracture单元
            (所以，大部分的属性应该会得到保留).
            3. 在此函数执行之后，对于每一个裂缝单元，都将有一个Cell来对应，
            且每一个Cell最多只和一个fracture对应。
        """
        assert isinstance(seepage, Seepage)
        assert isinstance(network, FractureNetwork2)
        core.hf2_alg_update_seepage_topology(
            seepage.handle, network.handle,
            fa_id,
    ...
    core.use(c_bool, 'hf2_alg_seepage_topology_expired',
             c_void_p, c_size_t)
    ...
    def seepage_topology_expired(network, fa_id):
        """
        判断裂缝对应的渗流是否已经过期
        （1、两个fracture共用一个cell，
        2、存在尚未设置cell的fracture）
        since 2023-9-24
        """
        assert isinstance(network, FractureNetwork2)
        return core.hf2_alg_seepage_topology_expired(network.handle, fa_id)

    core.use(None, 'hf2_alg_update_seepage_cell_pos',
             c_void_p, c_void_p,
             c_void_p, c_size_t)
    ...
    def update_seepage_cell_pos(seepage, network, coord, fa_id):
        ...
        core.hf2_alg_update_seepage_cell_pos(
            seepage.handle, network.handle, coord.handle, fa_id)

    core.use(None, 'hf2_alg_update_cond0',
             c_void_p, c_void_p, c_double,
             c_size_t, c_double)
    core.use(None, 'hf2_alg_update_cond1',
             c_void_p, c_size_t, c_size_t,
             c_size_t, c_double)

    @staticmethod
    def update_cond(seepage, network=None, fa_id=None, fw_max=None,
                    layer_dh=None,
        ...
        if ca_g is not None and ca_l is not None and ca_h is not None:
            core.hf2_alg_update_cond1(
                seepage.handle, ca_g, ca_l, ca_h, fw_max)
        else:
            assert isinstance(network, FractureNetwork2)
            if layer_dh is None:
                layer_dh = -1  # 此时，将采用fracture的高度属性
            core.hf2_alg_update_cond0(
                seepage.handle, network.handle, fw_max,
                fa_id, layer_dh)

    core.use(None, 'hf2_alg_update_pore0',
             c_void_p, c_void_p, c_size_t)
    core.use(None, 'hf2_alg_update_pore1',
             c_void_p, c_size_t, c_size_t,
             c_double)
    core.use(None, 'hf2_alg_update_pore2',
             c_void_p, c_size_t, c_size_t,
             c_size_t, c_size_t, c_size_t)
    core.use(None, 'hf2_alg_update_pore3',
             c_void_p, c_size_t, c_size_t,
             c_double, c_double)

    @staticmethod
    def update_pore(seepage, manager=None, fa_id=None, ca_s=None, ca_ny=None,
                    dw=None, ca_l=None,
        ...
        其中：
            ...
            ca_mu: 定义Cell位置的泊松比
        注意：
            当指定manager的时候，则使用manager来更新，
            否则，则使用seepage中定义的ca_s，ca_ny以及
            给定的dw来更新.
        """
        assert isinstance(seepage, Seepage)
        if (ca_ny is not None and ca_s is not None
                and k1 is not None and k2 is not None):
            # 此时建议的取值: k1=1.0e-10, k2=1.0e-8
            core.hf2_alg_update_pore3(
                seepage.handle, ca_ny, ca_s, k1, k2)
            return

        if isinstance(manager, InfManager2):
            assert fa_id is not None
            core.hf2_alg_update_pore0(
                seepage.handle, manager.handle, fa_id)
            return
        ...
        if (ca_l is not None and ca_h is not None and ca_ny is not None
                and ca_g is not None and ca_mu is not None):
            core.hf2_alg_update_pore2(
                seepage.handle, ca_l, ca_h, ca_ny, ca_g,
                ca_mu)
            return

        assert False

    core.use(None, 'hf2_alg_update_area',
    ...
    def update_area(seepage, ca_s, network, fa_id, fh=None):
        ...
        core.hf2_alg_update_area(
            seepage.handle, ca_s, network.handle, fa_id, fh)

    core.use(None, 'hf2_alg_update_length',
             c_void_p, c_size_t, c_void_p,
             c_size_t)
    ...
    def update_length(seepage, ca_l, network, fa_id):
        ...
        assert isinstance(seepage, Seepage)
        assert isinstance(network, FractureNetwork2)
        core.hf2_alg_update_length(
            seepage.handle, ca_l, network.handle, fa_id)

    core.use(None, 'hf2_alg_update_normal_stress',
             c_void_p, c_size_t, c_void_p,
             c_size_t, c_bool, c_double)

    @staticmethod
    def update_normal_stress(seepage, ca_ny, manager, fa_id, except_self=True,
                             relax_factor=1.0):
        """
        更新裂缝的法向应力(以拉应力为正)，并存储在裂缝对应Cell的ca_ny属性上.
        注意:
            这个函数会首先删除掉seepage中所有的Cell的ca_ny属性，
            然后在遍历所有的裂缝，计算应力，然后给定裂缝的fa_id属性
            找到对应的Cell，然后在Cell上添加ca_ny属性. 因此，
            这个函数运行之后，只要某个Cell定义了ca_ny属性，那么ca_ny
            对应的数值就一定是最新的.
        注意：
        ...
        core.hf2_alg_update_normal_stress(
            seepage.handle, ca_ny, manager.handle,
            fa_id, except_self, relax_factor)

    core.use(None, 'hf2_alg_exchange_fluids',
             c_void_p, c_void_p, c_double,
             c_size_t, c_size_t)
    ...
    def exchange_fluids(layers, pipe, dt, ca_g, ca_fp=None):
        ...
        core.hf2_alg_exchange_fluids(
            layers.handle, pipe.handle, dt, ca_g, ca_fp)

    core.use(c_bool, 'hf2_alg_rect_v3_intersected',
             c_double, c_double,
             c_double, c_double, c_double, c_double,
    ...
    core.use(None, 'hf2_alg_create_links',
             c_void_p, c_void_p, c_size_t,
             c_size_t, c_size_t, c_size_t,
             c_size_t, c_size_t, c_size_t,
             c_double, c_double, c_double, c_double, c_double, c_double)
    ...
    def create_links(seepage, cell_n, keys, v3, buf=None):
        """
        对于seepage模型，读取cell中定义的竖直的矩形的信息，
        返回和给定的矩形相交的所有的Cell的ID
        """
        if buf is None:
            buf = UintVector()
        assert isinstance(seepage, Seepage)
        assert len(v3) == 6
        core.hf2_alg_create_links(
            buf.handle, seepage.handle, cell_n,
...
class FracScatter3(HasHandle):
    ...
    def __init__(self, path=None, handle=None):
        super(FracScatter3, self).__init__(
            handle, core.new_fsc3, core.del_fsc3)
        if handle is None:
            if path is not None:
                self.load(path)
    ...
    def save(self, path):
        ...
        序列化保存. 可选扩展名:
            ...
            2: .xml  xml格式 (具体一定可读性，体积最大，读写最慢，跨平台)
            3: .其它  二进制格式
                (速度最快，体积最小，但Windows和Linux下生成的文件不能互相读取)
        """
        if path is not None:
            core.fsc3_save(self.handle, make_c_char_p(path))

    core.use(None, 'fsc3_load',
             c_void_p, c_char_p)
    ...
    core.use(None, 'fsc3_write_fmap',
             c_void_p, c_void_p, c_char_p)
    core.use(None, 'fsc3_read_fmap',
             c_void_p, c_void_p, c_char_p)
    ...
    def set(self, idx, center, p0, p1):
        """
        设置一个裂缝，并且自动生成它的散点.
        其中center为三维矩形的中心点坐标, p0和p1分别为两个相邻的边的中心点的坐标.
        """
        assert len(center) == 3
        assert len(p0) == 3
        assert len(p1) == 3
    ...
    core.use(c_size_t, 'fsc3_get_intersection',
             c_void_p, c_size_t, c_size_t)
...
class Hf3Alg:
    ...
    """
    core.use(None, 'hf3_alg_set_nf_zero_ikr',
             c_void_p, c_size_t, c_size_t,
             c_size_t)
    ...
    core.use(None, 'hf3_alg_find_intersected_nfs',
             c_void_p, c_void_p, c_size_t,
             c_size_t,
    ...
    @staticmethod
    def find_intersected_nfs(model, nf_n, hf_id, ca_x1, ca_y1, ca_z1, ca_x2,
                             ca_y2, ca_z2, leps=None, buffer=None):
        """
        ...
        if leps is None:
            ...
            assert 0 < leps < 1.0e3
        core.hf3_alg_find_intersected_nfs(
            buffer.handle, model.handle, nf_n,
            hf_id, leps,
            ca_x1, ca_y1, ca_z1, ca_x2, ca_y2,
            ca_z2)
        return buffer

    core.use(None, 'hf3_alg_get_new_cells',
             c_void_p, c_void_p, c_size_t)
    ...
    def get_new_cells(model, ca_is_new, buffer=None):
        ...
        if not isinstance(buffer, UintVector):
            buffer = UintVector()
        assert isinstance(model, Seepage)
        core.hf3_alg_get_new_cells(
            buffer.handle, model.handle, ca_is_new)
        return buffer

    core.use(None, 'hf3_alg_update_pore',
             c_void_p, c_double, c_double,
             c_void_p,
             c_size_t, c_size_t, c_size_t,
             c_size_t, c_size_t, c_size_t)
    ...
    @staticmethod
    def update_pore(model, thick, pore_modulus, cell_ids, ca_x1, ca_y1, ca_z1,
                    ca_x2, ca_y2, ca_z2):
        """
        更新孔隙空间
        """
        ...
        core.hf3_alg_update_pore(
            model.handle, thick, pore_modulus,
            cell_ids.handle,
            ca_x1, ca_y1, ca_z1, ca_x2, ca_y2, ca_z2)

    core.use(None, 'hf3_alg_set_attr',
    ...
    def set_attr(model, index, value, cell_ids):
        ...
        assert isinstance(model, Seepage)
        assert isinstance(cell_ids, UintVector)
        core.hf3_alg_set_attr(
            model.handle, index, value, cell_ids.handle)

    core.use(None, 'hf3_alg_update_rc3',
             c_void_p,
    ...
    def update_rc3(flow, network, fixed_n, layer_n, thick,
        ...
        assert isinstance(network, FractureNetwork2)
        core.hf3_alg_update_rc3(
            flow.handle, network.handle, fixed_n, layer_n,
            thick,
            ca_x1, ca_y1, ca_z1, ca_x2, ca_y2, ca_z2)

    core.use(None, 'hf3_alg_update_pc',
             c_void_p, c_void_p,
             c_void_p, c_void_p,
             c_double, c_double, c_double, c_double, c_double, c_double,
             c_size_t, c_size_t, c_size_t, c_size_t, c_size_t, c_size_t,
    ...
    def update_pc(model, cell_ids, insitu_stress, strength, left, right,
        ...
        assert len(left) == 3
        assert len(right) == 3
        core.hf3_alg_update_pc(
            model.handle, cell_ids.handle,
            insitu_stress.handle, strength.handle,
            left[0], left[1], left[2], right[0], right[1],
            right[2],
            ca_x1, ca_y1, ca_z1, ca_x2, ca_y2, ca_z2, ca_pc)

    core.use(None, 'hf3_alg_classify_cells',
             c_void_p, c_size_t, c_size_t,
             c_size_t)
    ...
    def classify_cells(model, fixed_n, ca_tag=None, ca_pc=None):
        ...
        core.hf3_alg_classify_cells(
            model.handle, fixed_n, ca_tag, ca_pc)

    core.use(None, 'hf3_alg_classify_faces',
             c_void_p, c_size_t, c_size_t)
    ...
    core.use(None, "hf3_alg_update_cell_len",
             c_void_p, c_void_p,
             c_size_t, c_size_t, c_size_t, c_size_t)

    @staticmethod
    def update_cell_len(model, network, ca_len=None, fa_id=None,
                        ma_layer_n=None, ma_fixed_n=None):
        assert isinstance(model, Seepage)
        assert isinstance(network, FractureNetwork2)
        if ca_len is None:
        ...
        core.hf3_alg_update_cell_len(
            model.handle, network.handle, ca_len,
            fa_id, ma_layer_n, ma_fixed_n)

    core.use(None, 'hf3_alg_update_face_geometry',
             c_void_p,
             c_size_t, c_size_t, c_size_t,
             c_double,
             c_size_t, c_size_t,
    ...
    @staticmethod
    def update_face_geometry(
            model, fa_s=None, fa_l=None, ca_len=None,
            pore_thick=None, fixed_n=None, fixed_face_n=None,
            layer_n=None, z0=None, z1=None):
        """
        ...
        if fixed_face_n is None:
            fixed_face_n = round(
                model.get_attr(model.get_model_key('fixed_face_n')))
        if layer_n is None:
        ...
        core.hf3_alg_update_face_geometry(
            model.handle, fa_s, fa_l, ca_len,
            pore_thick,
            fixed_n, fixed_face_n, layer_n, z0,
            z1)

    @staticmethod
    def update_seepage_topology(
            seepage, fixed_n, layer_n, network, fa_id,
            fa_new, z_range,
        ...
        计算主要分为3步：
            ...
            3、将更新了结构的Cell和Face再重新压入到seepage中，
            并且建立这些层之间的连接(Face)
        ...
        注意：
            如果调整了结构(会首先判断结构是否过期)，那么对于活动的cell，
            其额外添加的那些face将会被删除掉.
        """
        assert isinstance(network, FractureNetwork2)
        assert isinstance(seepage, Seepage)
        ...
        for layer_i in range(layer_n):
            ...
            kwargs = dict(ibeg0=0, other=seepage, ibeg1=ibeg,
                          count=layer_cell_n)
        ...
        for layer_i in range(layer_n):
            ...
            lay = layers[layer_i]
            Hf2Alg.update_seepage_topology(
                seepage=lay, network=network,
                fa_id=fa_id,
                fa_new=fa_new, cell_new=cell_new,
                face_new=face_new
            )
        ...
        layer_z = [z_min + layer_h * (layer_i + 0.5) for layer_i in
                   range(layer_n)]
        new_n = 0  # 新的裂缝单元的数量
        for frac in network.get_fractures():
    ...
    core.use(None, 'hf3_alg_cell_ini',
             c_void_p, c_void_p,
             c_size_t, c_size_t, c_size_t,
    ...
    def cell_ini(seepage, cell_ids, ca_vol, ca_t, ca_mc, fa_t, fa_c,
        ...
        对给定序号的Cell进行初始化
        """
        core.hf3_alg_cell_ini(
            seepage.handle, cell_ids.handle, ca_vol,
            ca_t, ca_mc, fa_t, fa_c, temperature)

    core.use(None, 'hf3_alg_face_ini',
             c_void_p, c_size_t, c_size_t,
    ...
    def face_ini(seepage, i_beg, i_end, fa_s, fa_l, fa_tag, hf_perm,
                 ratio=10.0):
        core.hf3_alg_face_ini(
            seepage.handle, i_beg, i_end, fa_s, fa_l, fa_tag,
            hf_perm, ratio)

    core.use(c_size_t, 'hf3_alg_count_cells',
             c_void_p, c_size_t, c_double,
             c_double)
    ...
    def count_cells(seepage, ca_tag, tag, eps=0.1):
        ...
        返回tag等于给定值的Cell的数量
        """
        return core.hf3_alg_count_cells(
            seepage.handle, ca_tag, tag, eps)

    core.use(None, 'hf3_alg_update_area_by_rc3',
             c_void_p, c_size_t,
             c_size_t, c_size_t, c_size_t,
             c_size_t, c_size_t, c_size_t,
             c_void_p)
    ...
    @staticmethod
    def update_area_by_rc3(
            seepage, ca_s, ca_x1, ca_y1, ca_z1, ca_x2, ca_y2,
            ca_z2, cell_ids=None):
        """
        根据rc3属性来计算面积
        """
        assert isinstance(seepage, Seepage)
        if cell_ids is None:
            core.hf3_alg_update_area_by_rc3(
                seepage.handle,
                ca_s, ca_x1, ca_y1, ca_z1, ca_x2,
                ca_y2, ca_z2, 0)
        else:
            assert isinstance(cell_ids, UintVector)
            core.hf3_alg_update_area_by_rc3(
                seepage.handle,
                ca_s, ca_x1, ca_y1, ca_z1, ca_x2,
                ca_y2, ca_z2, cell_ids.handle)


class Sigma3:
    core.use(None, 'sigma3_get1',
             c_void_p,
             c_double, c_double, c_double,
             c_double, c_double, c_double,
             c_double, c_double, c_double)

    core.use(None, 'sigma3_get2',
             c_void_p,
             c_double, c_double, c_double,
             c_double, c_double, c_double,
             c_double, c_double, c_double,
    ...
    def get(pos, disp, G, mu, area=None, triangle=None, buffer=None):
        ...
        if not isinstance(buffer, Tensor3):
            buffer = Tensor3()
        if area is not None:
            core.sigma3_get1(buffer.handle, pos[0], pos[1], pos[2], disp[0],
                             disp[1], disp[2], area, G, mu)
            return buffer
        else:
