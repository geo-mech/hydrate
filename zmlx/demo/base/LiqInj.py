...
def create():
    ...
    config.set(disable_update_den=True, disable_update_vis=True, disable_ther=True, disable_heat_exchange=True)
    ...
    model = config.create(mesh, porosity=0.2, pore_modulus=100e6, p=1e6, temperature=280,
                          s=(1, 0), perm=1e-14)
...
def solve(config, model):
    iterate = GuiIterator(iterate=config.iterate, plot=lambda: show(config, model))

    while config.get_time(model) < 365 * 24 * 3600:
        r = iterate(model)
        step = config.get_step(model)
        if step % 10 == 0:
            dt, t = time2str(config.get_dt(model)), time2str(config.get_time(model))
