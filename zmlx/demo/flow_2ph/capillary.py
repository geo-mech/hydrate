...
def create(jx, jy):
    ...
    mesh = create_cube(np.linspace(-50, 50, jx + 1),
...
def show(x, y, z, jx, jy, caption=None, cmap=None):
    angles = np.linspace(0, 2 * np.pi, 100)
    items = [
        item('contourf', np.reshape(x, shape=[jx, jy]), np.reshape(y, shape=[jx, jy]),
             np.reshape(z, shape=[jx, jy]), cmap=cmap),
    ...
    plot(add_axes2, add_items, *items, aspect='equal', xlabel='x', ylabel='y', caption=caption, tight_layout=True)
...
def solve(model: Seepage, jx, jy):
    ...
    show(x, y, [get_s(x[i], y[i], z[i])[1] for i in range(len(x))],
         jx, jy, caption='初始饱和度', cmap='coolwarm')
    ...
    for step in range(500):
        ...
        if step % 20 == 0:
            print(f'step = {step}')
            show(x, y, seepage.get_v(model, 1), jx, jy, caption='实时饱和度', cmap='coolwarm')
