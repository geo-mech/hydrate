# ** desc = '单相的流动，在上一个case的基础上，增加了渗透率的随机性）'
import random
...
def test():
    ...
    for ix in range(jx):
        for iy in range(jy):
            ...
            if ix < jx - 1:
                add_face(
                    model, idx, idx + jy, area=1, dist=1,
                    perm=1.0e-15 * random.uniform(0.5, 1.5) * 5
                )
            if iy < jy - 1:
                add_face(
                    model, idx, idx + 1, area=1, dist=1,
                    perm=1.0e-15 * random.uniform(0.5, 1.5)
                )
