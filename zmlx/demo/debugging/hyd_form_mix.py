...
from zml import SeepageMesh, get_distance, create_dict, Seepage, core, ConjugateGradientSolver
...
def create():
    ...
    mesh = SeepageMesh.create_cube(x=np.linspace(-150, 150, 150),
                                   y=(-0.5, 0.5),
    ...
    kw.update(create_dict(mesh=mesh, porosity=0.1,
                          pore_modulus=100e6,
...
def solve(model, folder=None, step_max=None):
    ...
    iterate = GuiIterator(lambda m: seepage.iterate(m, solver=solver),
    ...
    save = SaveManager(join_paths(folder, 'seepage'),
                       save=model.save,
                       ext='.seepage',
                       time_unit='y',
