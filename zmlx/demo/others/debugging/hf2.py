...
def iterate():
    ...
    Hf2Alg.update_seepage_cell_pos(
        seepage=seepage, network=network,
        fa_id=frac_keys(seepage).id, coord=Coord3())
    ...
    # 更新流体
    Hf2Alg.update_pore(
        seepage=seepage, manager=manager,
        fa_id=frac_keys(seepage).id)
    Hf2Alg.update_cond(
        seepage=seepage, network=network,
    ...
    network.update_boundary(
        seepage=seepage, fa_id=frac_keys(seepage).id,
        fh=thick)
    manager.update_disp()
    network.extend(
        kic=kic, sol2=sol2,
