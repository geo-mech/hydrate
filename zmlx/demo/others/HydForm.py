...
def create():
    ...
    model = config.create(
        mesh, porosity=0.1, pore_modulus=100e6, denc=get_denc,
        dist=0.1,
...
def show(config, model, folder=None):
    ...
    kwargs = {'gui_only': True,
              'title': f'plot when model.time={time2str(time)}'}
    ...
    tricontourf(x, y, model.numpy.cells.get(config.cell_keys['pre']),
                caption='压力',
                fname=fname('pressure'), **kwargs)
    tricontourf(x, y, model.numpy.cells.get(config.cell_keys['temperature']),
                caption='温度',
    ...
    tricontourf(x, y, vs / vv, caption='水合物饱和度', fname=fname('hyd'),
                **kwargs)
...
def solve(config, model, folder=None):
    ...
    执行求解，并将结果保存到指定的文件夹
    """
    iterate = get_gui_iter(iterate=config.iterate, plot=lambda: show(
        config, model,
    ...
    save = SaveManager(
        join_paths(folder, 'seepage'), save=model.save, ext='.seepage',
        time_unit='y',
...
def execute(gui_mode=True, close_after_done=False):
    ...
    if gui_mode:
        gui.execute(solve, close_after_done=close_after_done,
                    args=(config, model, opath('HydForm')))
    else:
        solve(config, model, opath('HydForm'))
